...
ShopResult shopCalc({
  ...
  if (direction == 'cashin') {
    // Recharge → commission %. UPI transfer → round-off charge.
    final income = commPct > 0
        ? r2(amount * commPct / 100)
        : customerCharge(amount, chargePct, roundTo);
    return ShopResult(
      payout: commPct > 0 ? income : 0,
      tds: 0,
      charge: commPct > 0 ? 0 : income,
      net: income,
      walletDelta: -amount,
      cashDelta: amount,
    );
  ...
  final tds = r2(payout * tdsPct / 100);
  final charge = customerCharge(amount, chargePct, roundTo);
  return ShopResult(
    payout: payout,
    tds: tds,
    charge: charge,
