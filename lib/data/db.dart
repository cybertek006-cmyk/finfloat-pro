...
class DB {
  ...
  /// Test ke liye override. Production mein hamesha null rehta hai,
  /// isliye asli app par koi asar nahi.
  String? testPath;

  Future<String> path() async =>
      testPath ?? p.join(await getDatabasesPath(), fileName);
  ...
  Future<Database> _open() async {
    return openDatabase(
      ...
      onUpgrade: (d, from, to) async {
        // v1 -> v2: do cash box + day-end summary
        if (from < 2) {
          try {
            await d.execute('''CREATE TABLE IF NOT EXISTS cash_transfers(
              id INTEGER PRIMARY KEY AUTOINCREMENT,
              from_box TEXT NOT NULL, to_box TEXT NOT NULL,
        ...
        // v2 -> v3: distributor ID se retailer funding
        if (from < 3) {
          try {
            await d.execute(
                'ALTER TABLE accounts ADD COLUMN funds_retailers INTEGER NOT NULL DEFAULT 0');
          } catch (_) {}
          try {
            await d.execute('ALTER TABLE retailers ADD COLUMN account_id INTEGER');
          } catch (_) {}
          // Purani entries: jis distributor ID se pehle se issue hue hain,
          // usko apne aap "funding karta hai" mark kar dein.
          try {
            await d.execute('''UPDATE accounts SET funds_retailers = 1
              WHERE id IN (SELECT DISTINCT account_id FROM retailer_issues)''');
          } catch (_) {}
          // Retailer ko uski sabse zyada use hui ID se map kar dein.
          try {
            await d.execute('''UPDATE retailers SET account_id = (
              SELECT account_id FROM retailer_issues
  ...
  static const List<String> _schema = [
    ...
    '''CREATE TABLE accounts(
      ...
      -- Distributor ID hai to: kya is ID se retailers ko funding dete hain?
      funds_retailers INTEGER NOT NULL DEFAULT 0,
    ...
    '''CREATE TABLE deposits(
      ...
      cash_source TEXT NOT NULL DEFAULT 'counter',
    ...
    // Cash transfer -- mere cash aur counter cash ke beech
    ...
    // Purana float table (backward compatibility ke liye rakha)
    ...
    '''CREATE TABLE retailers(
      ...
      -- Ye retailer kis Distributor ID ke andar map hai (jaise A2Z Distributor)
      account_id INTEGER,
