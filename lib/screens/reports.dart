...
class _ReportsTabState extends State<ReportsTab> {
  ...
  Future<_Rep> _build() async {
    ...
    switch (_kind) {
      ...
      // Party-wise -- har party ka apna total, rate ke saath
      case 'cmsparty':
        final parties = await r.parties();
        final all = await r.cmsEntries(date: d);
        final rows = <List<String>>[];
        for (final p in parties) {
          final mine =
              all.where((x) => x['party_id'] == p['id']).toList();
          if (mine.isEmpty) continue;
          final rate = p['mode'] == 'percent'
              ? '${numOf(p['rate'])}%'
              : '₹${numOf(p['rate'])}';
          rows.add([
            '${p['name']}',
            ...
            '${numOf(p['tds_pct'])}%',
            '${mine.length}',
            sm(sumOf(mine, 'amount')),
            money(sumOf(mine, 'payout')),
            money(sumOf(mine, 'tds')),
            money(sumOf(mine, 'net')),
          ]);
        }
        return _Rep(
          'CMS Party-wise Report',
          ['Party', 'Rate', 'TDS%', 'Pickups', 'Volume', 'Payout', 'TDS', 'Net'],
          rows,
          {
            'Active parties': '${rows.length}',
            'Total volume': money(sumOf(all, 'amount')),
            'Total payout': money(sumOf(all, 'payout')),
            'Total TDS': money(sumOf(all, 'tds')),
            'NET COMMISSION': money(sumOf(all, 'net')),
          },
        );
      ...
      case 'cash':
        ...
        final myDep = dep.where((x) => x['cash_source'] == 'mine').toList();
        final ctrDep = dep.where((x) => x['cash_source'] != 'mine').toList();
        return _Rep('Cash Flow Report', ['Type', 'In', 'Out'], [
          ['CMS collection (mera cash)', money(await r.cmsVolume(d)), '—'],
          ...svcRows,
          ['Cash float/transfer', money(sumOf(fl, 'amount')), '—'],
          ['Deposit (mere cash se)', '—', money(sumOf(myDep, 'amount'))],
          ['Deposit (counter se)', '—', money(sumOf(ctrDep, 'amount'))],
        ], {
          '👤 Mera Cash': money(await r.myCash()),
          '🏪 Counter Cash': money(await r.counterCash()),
          'TOTAL CASH': money(await r.totalCash()),
        });
