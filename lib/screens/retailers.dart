                ...
                return Tile(
                  ...
                  onMenu: () async {
                    ...
                    if (x == 'edit' && mounted) {
                      ...
                      if (await confirm(context, 'Delete retailer?',
...
class _RetailerFormState extends State<RetailerForm> {
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: ListView(padding: const EdgeInsets.all(14), children: [
          F(_name, 'Retailer Name *', hint: 'e.g. Sharma Kirana'),
  ...
  Future<void> _save() async {
    ...
    final row = {
      ...
      'credit_limit': parseD(_limit) <= 0 ? 30000.0 : parseD(_limit),
    };
    widget.edit == null
        ? await Repo.i.add('retailers', row)
        : await Repo.i.edit('retailers', widget.edit!['id'] as int, row);
...
class _IssueFormState extends State<IssueForm> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_accs.isEmpty) {
      return Scaffold(
        appBar: AppBar(title: const Text('Fund Dein')),
        body: const Empty('🆔', 'Koi Company ID nahi',
            sub: 'More → Company IDs se add karein'),
      );
    }
