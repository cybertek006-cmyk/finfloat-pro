    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(14), children: [
        ...
        F(_cnt, 'Kitne transactions?',
            type: TextInputType.number, intOnly: true, onChanged: (_) => _recalc()),
        if (c != null)
          Container(
            padding: const EdgeInsets.all(12),
            margin: const EdgeInsets.only(bottom: 12),
...
class _DepositFormState extends State<DepositForm> {
  ...
  String _cashSource = 'mine'; // mine | counter
  ...
  void initState() {
    ...
    if (e != null) {
      ...
      _mode = '${e['mode']}';
      _cashSource = '${e['cash_source'] ?? 'counter'}';
    }
    _load();
  }
      ...
      body: _banks.isEmpty
          ...
          : ListView(padding: const EdgeInsets.all(14), children: [
              ...
              _dd<int>('Bank (charge lagega)', _bank,
                  ...
                  (v) => setState(() => _bank = v)),
              // Kaunse cash se deposit -- mera ya counter
              const Text('Kis cash se jama kiya?',
                  style: TextStyle(
              ...
              Row(children: [
                Expanded(
                  child: GestureDetector(
                    onTap: () => setState(() => _cashSource = 'mine'),
                    child: Container(
                      padding: const EdgeInsets.all(11),
                      decoration: BoxDecoration(
                        color: _cashSource == 'mine'
                            ? const Color(0xFFF0F4FB)
                            : Colors.white,
                        border: Border.all(
                            color: _cashSource == 'mine' ? C.primary : C.divider,
                            width: _cashSource == 'mine' ? 1.6 : 1),
                        borderRadius: BorderRadius.circular(10),
                      ),
                      child: Column(children: [
                ...
                Expanded(
                  child: GestureDetector(
                    onTap: () => setState(() => _cashSource = 'counter'),
                    child: Container(
                      padding: const EdgeInsets.all(11),
                      decoration: BoxDecoration(
                        color: _cashSource == 'counter'
                            ? const Color(0xFFFFF8EC)
                            : Colors.white,
                        border: Border.all(
                            color: _cashSource == 'counter' ? C.warning : C.divider,
                            width: _cashSource == 'counter' ? 1.6 : 1),
                        borderRadius: BorderRadius.circular(10),
                      ),
                      child: Column(children: [
                        const Text('🏪 Counter',
