    ...
    try {
      if (_setup) {
        ...
        if (PinService.locked) {
          ...
          // Hindi welcome — await nahi karte taaki app turant khule.
          // Voice fail ho to bhi app rukti nahi.
          Voice.i.welcome();
          widget.onOk();
        } else {
          HapticFeedback.heavyImpact();
...
class _HomeState extends State<Home> {
  ...
  /// Subah ka opening bhara ya nahi -- bina bhare app use nahi hogi.
  ///
  /// FIX: pehle shart `accs.isNotEmpty` thi, yaani jab tak ek bhi Company ID
  /// add na ho gate aata hi nahi tha. Par gate ka matlab hi ye hai ki har
  /// din ka opening cash likha jaaye -- wo cash Company ID par nirbhar nahi
  /// karta. Isliye ab gate ROZ aata hai, chahe koi ID ho ya na ho.
  ///
  /// Ek hi chhoot: bilkul nayi app (koi company hi nahi bani) -- tab pehle
  /// user ko masters bharne dete hain, warna wo phas jaayega.
  Future<void> _checkGate() async {
    final done = await Repo.i.hasSnap('open');
    final comps = await Repo.i.companies();
    if (!mounted) return;
    setState(() {
      // Company bani hai + aaj ka opening nahi bhara -> gate lagega
      _needsOpening = !done && comps.isNotEmpty;
      _checked = true;
    });
    if (!_needsOpening) _startClosingReminder();
  }

  void _startClosingReminder() {
    // Raat 11 baje ke baad closing na bhara ho to reminder
    ClosingReminder.start(() => context);
  }

  @override
  void dispose() {
    ClosingReminder.stop();
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_needsOpening) {
      return OpeningGate(onDone: () {
        setState(() {
          _needsOpening = false;
          _version++;
        });
        _startClosingReminder();
      });
    }
