...
/// ₹ ke saath Hindi shabd bhi — bade amount par "feel" aati hai.
/// ₹1,15,308  +  (एक लाख पंद्रह हज़ार तीन सौ आठ रुपये)
class MoneyHi extends StatelessWidget {
  final num? value;
  final double size;
  final Color? color;
  final bool bold;
  /// Itne se chhote amount par Hindi line nahi dikhati (chhote number par
  /// shabd likhna faltu lagta hai). Default ₹1000.
  final num minForWords;
  const MoneyHi(this.value,
      {super.key,
      this.size = 15,
      this.color,
      this.bold = true,
      this.minForWords = 1000});

  @override
  Widget build(BuildContext c) {
    final v = (value ?? 0).toDouble();
    final showWords = v.abs() >= minForWords;
    return Column(
      ...
      children: [
        ...
        if (showWords)
          Padding(
            ...
            child: Text(hindiRupees(v, paise: false),
                textAlign: TextAlign.right,
                style: TextStyle(
                    fontSize: (size * 0.52).clamp(8.5, 11.0),
                    height: 1.25,
                    color: C.fade(color ?? C.muted, .65))),
          ),
      ],
    );
  }
...
class AppCard extends StatelessWidget {
  ...
  Widget build(BuildContext c) => Container(
        ...
        // leftEdge = card ke baayein taraf rangeen patti (colour strip).
        // Pehle ye param declare tha par render hi nahi hota tha — ab hota hai.
        child: leftEdge == null
...
/// Form text field
///
/// FIX: pehle `TextInputType.number` diya jaata tha — Android par uska keypad
/// mein DOT hota hi nahi, isliye 0.35 / 0.5 jaisi decimal rate feed nahi ho
/// paati thi. Ab number type apne aap `decimal: true` ban jaata hai aur ek
/// formatter sirf digits + ek dot allow karta hai.
///
/// Jahan pukka poora number chahiye (transaction count etc.) wahan
/// `intOnly: true` de dein.
class F extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext c) {
    final numeric = _isNum;
    return Padding(
      padding: const EdgeInsets.only(bottom: 12),
      child: TextField(
        controller: ctrl,
        keyboardType: numeric
            ? TextInputType.numberWithOptions(decimal: !intOnly, signed: false)
            : type,
...
/// Ek se zyada decimal point nahi lagne deta ("0.3.5" rok deta hai)
class _OneDot extends TextInputFormatter {
  @override
  TextEditingValue formatEditUpdate(TextEditingValue old, TextEditingValue nw) {
    if ('.'.allMatches(nw.text).length <= 1) return nw;
    return old;
  }
}
